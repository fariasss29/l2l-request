...
def get_saidas_catraca_df() -> pd.DataFrame:
    ...
    df = pd.DataFrame([{
        'NU_MATRICULA': '99882749',       
        'NM_PESSOA':    'JEFFERSON SILVA',
        'CD_EQUIPAMENTO': 11,
        'DS_EQUIPAMENTO': 'Externa Portaria',
        'DT_REQUISICAO': '2025-06-04 16:10:00'
    }])
    df['matricula_UP'] = df['NU_MATRICULA'].astype(str).str.strip().str.upper()
    return df
...
def main():
    ...
    if df_saidas.empty:
        print("Nenhuma saída simulada encontrada na LOG_ACESSO.")
        return
    print(f"Saídas na catraca (simuladas): {len(df_saidas)} registro(s).")
    ...
    for dispatch_id, loginid_UP in combos:
        ...
        try:
            dt_id = get_dispatchtechnician_id(int(dispatch_id), user_id)
        except Exception as e:
